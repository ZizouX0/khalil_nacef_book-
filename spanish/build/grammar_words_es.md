# The Eight Grammar Words This Book Uses

You don't need to have studied grammar to use this book. You do need eight words. Two minutes here and they
will never slow you down again. English example first, every time.

If you already know what a subject and an object are, skim the table and go to the box at the bottom. The box
is the one that matters.

| Word | What it means | In English | In Spanish |
|---|---|---|---|
| **noun** | a thing, a person, a place or an idea | *book, sister, city, freedom* | *el libro, la hermana, la ciudad* |
| **verb** | an action or a state | *to speak, to be, to have* | *hablar, ser, tener* |
| **infinitive** | the basic, unchanged form of a verb — the one in the dictionary | *to speak* | *hablar* (all Spanish infinitives end in **-ar, -er** or **-ir**) |
| **to conjugate** | to change a verb so it matches who is doing it | *I speak, he speak**s*** | *hablo, hablas, habla…* |
| **subject** | who or what does the verb | ***Ana** speaks Spanish* | ***Ana** habla español* |
| **object** | who or what the verb is done to | *Ana speaks **Spanish*** | *Ana habla **español*** |
| **gender** | every Spanish noun is either masculine or feminine — it has nothing to do with meaning | English has no gender | *el libro* (m.), *la mesa* (f.) |
| **agreement** | making the words around a noun match its gender and number | English barely does this | *l**a** cas**a** blanc**a**, l**os** libr**os** blanc**os*** |

**Two more you will meet, so here they are early:**

- **article** — the little word before a noun: *the* → **el, la, los, las**; *a/an* → **un, una**.
- **pronoun** — a word that replaces a noun so you don't repeat it: *I, you, it, them* → *yo, tú, lo, las*.

> **Gender is the one that trips everyone up.** In English a table is just a table. In Spanish it is *la
> mesa*, feminine, and everything you put around it has to agree: *la mesa pequeña*, never "el mesa pequeño".
> So never learn *mesa*. Learn **la mesa**, from your very first word. The article is part of the word.
> This book prints every noun with a coloured tag — <span class="gen m">el</span> for masculine,
> <span class="gen f">la</span> for feminine — so you can't quietly forget which is which.
